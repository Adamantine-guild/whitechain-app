...
export function VaultCardMobile({ vault, nativeBalance, onStake, onUnstake }: VaultCardMobileProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(val) || val <= 0) {
      setError(t('vaultCard.invalidAmount'));
      return;
    }

    if (val > maxAmount) {
      setError(t('vaultCard.insufficientBalance', { max: maxAmount.toFixed(4) }));
      return;
    }
  ...
  return (
    <article
      aria-label={t('vaultCard.stakeDetails', { name: vault.name })}
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400">{t('vaultCard.assetLabel')}: {vault.asset}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 border-t border-b border-gray-100 py-3 dark:border-gray-800">
        <div>
          <span className="text-xs text-gray-500 dark:text-gray-400">{t('vaultCard.tvl')}</span>
          <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{vault.tvl}</p>
        </div>
        <div>
          <span className="text-xs text-gray-500 dark:text-gray-400">{t('vaultCard.yourStake')}</span>
      ...
      {success && (
        <div className="flex items-center gap-2 rounded bg-green-50 p-3 text-xs text-green-700 dark:bg-green-950/20 dark:text-green-400" role="status">
          ...
          <span>{t('vaultCard.txSucceeded')}</span>
        </div>
      )}
      ...
      {activeForm && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded bg-gray-50 p-3 dark:bg-gray-800/40">
          <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
            <span className="capitalize">{t('vaultCard.amountLabel', { action: activeForm === 'stake' ? t('vaultCard.stake') : t('vaultCard.unstake') })}</span>
            <button
              type="button"
              onClick={() => setAmount(maxAmount.toString())}
              className="font-semibold text-blue-600 hover:underline dark:text-blue-400"
            >
              {t('vaultCard.useMax')} ({maxAmount.toFixed(4)})
            </button>
          </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span>{t('vaultCard.processing')}</span>
                </>
              ) : (
                <span>{activeForm === 'stake' ? t('vaultCard.confirmStake') : t('vaultCard.confirmUnstake')}</span>
              )}
            </button>
            <button
              type="button"
